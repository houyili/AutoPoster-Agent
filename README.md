...
### 2. Secure Your API Keys (macOS Keychain)
To protect your privacy and prevent accidental leaks of your OpenAI API Key, we use the macOS Keychain. Run the setup script to store your key securely:

```bash
python setup_keychain.py
```
*You will be prompted to paste your API Key. It will be stored securely in your system's Keychain under the service name `AutoPoster-Agent`.*

## Usage

### Injecting the SOP into your AI Agent
1. Open your preferred AI code editor (e.g., Cursor, VSCode with Copilot, or an agent framework).
2. Include the `templates/prompt_sops/agent_sop.md` file in the Agent's context.
3. Provide the Agent with your `outline.md` and any data spreadsheets.
